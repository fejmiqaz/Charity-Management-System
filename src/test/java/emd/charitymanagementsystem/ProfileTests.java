...
class ProfileTests {
 ...
 @BeforeEach void setup() {
  ...
  memberId=member.getId();accountId=account.getId();
 }
 ...
 @Test void showsAndUpdatesOwnProfileWithoutChangingPermissions() throws Exception {
  ...
  assertEquals("Updated",member.getName());assertEquals("Ohrid",member.getCity());assertEquals(Role.MEMBER,member.getRole());assertNull(member.getYear());
  var account=accounts.findById(accountId).orElseThrow();assertEquals(Role.MEMBER,account.getRole());assertEquals("encoded",account.getPassword());
 }
 @Test void changedEmailUpdatesBothRecordsAndEndsSession() throws Exception {
  String updated="new"+email;
  mvc.perform(change(updated)).andExpect(redirectedUrl("/login?profileUpdated"));
  assertEquals(updated,accounts.findById(accountId).orElseThrow().getEmail());assertEquals(updated,members.findById(memberId).orElseThrow().getEmail());
 }
 @Test void rejectsInvalidAndDuplicateEmail() throws Exception {
 ...
 @Test void accountWithoutMemberShowsHelpfulMessage() throws Exception {
