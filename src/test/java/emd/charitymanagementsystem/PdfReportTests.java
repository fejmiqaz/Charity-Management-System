...
class PdfReportTests {
    @Test
    void rendersEveryReportAndMultipagePreview() throws Exception {
        var service = new PdfExportService();
        var project = new ProjectResponseDto();
        project.setId(1L);
        project.setName("Community learning and outreach");
        project.setDescription("Provide learning materials and coordinate volunteer-led activities for the local community.");
        project.setStatus(ProjectStatus.ONGOING);
        project.setProjectPrice(1250.50);
        project.setDateCreated(LocalDate.of(2026, 9, 1));
        project.setMemberNames(List.of("Elena Marku", "Arben Dervishi"));
        var budget = new BudgetResponseDto();
        budget.setId(1L);
        budget.setBudgetAmount(15000.0);
        budget.setDescription("Annual allocation for community programs and events.");
        var donation = new DonationResponseDto();
        donation.setId(1L);
        donation.setDonationAmount(2500.0);
        donation.setMemberNames(List.of("Elena Marku"));
        var event = new EventResponseDto();
        event.setId(1L);
        event.setPurpose("Community volunteer day and project planning workshop");
        event.setDate(LocalDateTime.of(2026, 9, 25, 10, 30));
        event.setMemberNames(List.of("Elena Marku", "Arben Dervishi"));
        var year = new Years();
        year.setId(1L);
        ...
        for (var sample : samples.entrySet()) {
            ...
            try (var reader = new PdfReader(sample.getValue())) {
                ...
                for (int page = 1; page <= reader.getNumberOfPages(); page++) {
                    ...
                    assertTrue(pageText.contains("Page " + page), sample.getKey());
                    text.append(pageText);
                }
                assertTrue(text.toString().contains("EXPORT DATE"), sample.getKey());
                assertTrue(text.toString().contains("UTC"));
                assertTrue(text.toString().contains("Signature:"));
                assertTrue(text.toString().contains("REVIEWED / APPROVED BY"));
                if (sample.getKey().equals("projects")) {
                    assertTrue(reader.getNumberOfPages() > 1);
                    assertTrue(text.toString().contains("1,250.50 EUR"));
                }
            }
        }
    }
