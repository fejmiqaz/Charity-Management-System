...
class MemberDeletionTests {
    ...
    @Test
    void onlyHeadCanDeleteThroughMemberEndpoint() throws Exception {
    ...
    @Test
    void deletesMemberLinkedToAccountAndYearAfterCommit() {
    ...
    @Test
    void removesAssociationsWithoutDeletingOtherRecords() {
        Long id = new TransactionTemplate(transactions).execute(status -> {
            ...
            budget.setMembers(new ArrayList<>(List.of(member)));
            budgets.save(budget);
            Donation donation = new Donation();
            donation.setMembers(new HashSet<>(Set.of(member)));
            donations.save(donation);
            Event event = new Event();
            event.setMembers(new ArrayList<>(List.of(member)));
            events.save(event);
            Project project = new Project();
            project.setMembers(new HashSet<>(Set.of(member)));
        ...
        assertEquals(budgetCount, budgets.count());
        assertEquals(donationCount, donations.count());
        assertEquals(eventCount, events.count());
        assertEquals(projectCount, projects.count());
    }
}
