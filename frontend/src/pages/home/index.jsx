...
export default function Home() {
  ...
  return <div className="public-home" id="top">
    ...
    <main id="home-main">
      <section className="home-hero"><div className="home-wrap home-hero-grid"><div><span className="home-eyebrow"><i /> {t("Small actions. Lasting change.")}</span><h1>{t("Good work starts with")}<br /><em>{t("people.")}</em></h1><p>{t("Discover the projects we have completed and the moments that bring our community together.")}</p><div className="home-actions"><a className="home-button home-primary" href="#projects">{t("Explore our work")} <span>↗</span></a><a className="home-text-link" href="#events">{t("Upcoming events")} ↓</a></div></div><div className="home-visual"><div className="home-visual-top"><span>{t("OUR COMMUNITY, IN MOTION")}</span><span>✳ {t("Together")}</span></div><div className="home-art" aria-hidden="true"><div className="home-orbit orbit-one" /><div className="home-orbit orbit-two" /><div className="home-art-heart">♥</div><span className="home-art-star">✳</span><span className="home-art-caption">{t("A little care.")}<br />{t("A lasting difference.")}</span></div><div className="home-next"><span className="home-eyebrow">{t(d?.events?.length ? 'Next public event' : 'More to come')}</span><h2>{d?.events?.[0]?.purpose || t('Room for the next good thing.')}</h2><p>{d?.events?.length ? <><EventDate date={d.events[0].date} /> · {d.publicZone}</> : r.loading ? t('Loading community updates…') : r.error ? t('Community updates are temporarily unavailable.') : t('New public events will appear here when announced.')}</p></div></div></div>
