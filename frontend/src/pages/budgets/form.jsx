...
export default function BudgetForm() {
  ...
  const r = useResource(async () => {
    if (edit) setF(await budgetApi.get(yearId));
    return true;
  }, [yearId, edit]);
  async function submit(e) {
    ...
    try {
      const body = {
        ...f,
        budgetAmount: Number(f.budgetAmount)
      };
      edit ? await budgetApi.update(yearId, body) : await budgetApi.create(yearId, body);
      nav(`/years/${yearId}/budget`);
    } catch (e) {
      setError(e);
    } finally {
      setBusy(false);
